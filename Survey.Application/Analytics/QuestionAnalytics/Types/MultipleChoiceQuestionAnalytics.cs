...
{
    public class MultipleChoiceQuestionAnalytics : IAnalyticsType
    {
        private readonly ISurveyDbContext _dbContext;

        public MultipleChoiceQuestionAnalytics(ISurveyDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public virtual QuestionType Type => QuestionType.MultipleChoice;

        public async Task<IEnumerable<AnalyticsQuestionViewModel>> GetAnalytics(int id, int surveyResultId)
        {
            return await _dbContext.CheckBoxQuestionResponses
